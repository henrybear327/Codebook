...
bool CircleInsidePolygon(int vcount, POINT center, double radius,
                         POINT polygon[])
{
    POINT q;
    double d;
...
/* 返回两个矢量l1和l2的夹角的余弦(-1 ---
 * 1)注意：如果想从余弦求夹角的话，注意反余弦函数的定义域是从 0到pi */
...
//
//判断P1P2跨立Q1Q2的依据是：( P1 - Q1 ) × ( Q2 - Q1 ) * ( Q2 - Q1 ) × ( P2 - Q1
//) >= 0。
//判断Q1Q2跨立P1P2的依据是：( Q1 - P1 ) × ( P2 - P1 ) * ( P2 - P1 ) × ( Q2 - P1
//) >= 0。
...
{
    return ((intersect(u, v)) && (!online(u, v.s)) && (!online(u, v.e)) &&
...
void checkconvex(int vcount, POINT polygon[], bool bc[])
{
    ...
    bc[index] = 1;
    while (count) { // 判断凸凹性
        if (multiply(polygon[(index + 1) % vcount], polygon[(index + 2) % vcount],
                     polygon[index]) >= 0)
            bc[(index + 1) % vcount] = 1;
        else
            bc[(index + 1) % vcount] = 0;
            ...
            ((bonline1 = online(l1, Polygon[(i + 1) % n])) && // 第二个端点在射线上
             ((!(bonline2 = online(
              ...
              ((r1 = multiply(Polygon[i], Polygon[(i + 1) % n], l1.s) *
                     multiply(Polygon[(i + 1) % n], Polygon[(i + 2) % n],
                              l1.s)) > 0) ||
              (bonline3 = online(
                              l1,
                              Polygon
...
bool InsideConvexPolygon(int vcount, POINT polygon[], POINT q) // 可用于三角形！
{
    ...
    p.y = 0;
    for (i = 0; i < vcount;
         i++) { // 寻找一个肯定在多边形polygon内的点p：多边形顶点平均值
        p.x += polygon[i].x;
        p.y += polygon[i].y;
    }
    p.x /= vcount;
          ...
          */
    {
        ...
        for (j = i + 1; j < n; j++)
            if (multiply(PointSet[j], PointSet[k], PointSet[0]) > 0 || // 极角更小
                (multiply(PointSet[j], PointSet[k], PointSet[0]) ==
                 0) && /* 极角相等，距离更短 */
...
void ConvexClosure(POINT PointSet[], POINT ch[], int n, int &len)
{
    ...
    for (i = 1; i < n; i++) {
        if (PointSet[i].y < tmp.y ||
            PointSet[i].y == tmp.y && PointSet[i].x < tmp.x) {
            index = i;
        }
        use[i] = false;
    }
    ...
    while (index != first) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (curcos > curmax ||
                fabs(curcos - curmax) < 1e-6 && dist(l2.s, l2.e) > curdis) {
                curmax = curcos;
                index = i;
...
{
    // 判断线端l的端点是否不都在多边形内
    if (!insidepolygon(vcount, polygon, l.s) ||
        !insidepolygon(vcount, polygon, l.e))
        return false;
    int top = 0, i, j;
    ...
    for (i = 0; i < top - 1;
         i++) { /* 冒泡排序，x坐标小的排在前面；x坐标相同者，y坐标小的排在前面 */
        for (j = i + 1; j < top; j++) {
            if (PointSet[i].x > PointSet[j].x ||
                fabs(PointSet[i].x - PointSet[j].x) < EP &&
...
但要注意函数的声明和调用要做相应变化
**********************************************************************************************/
void AddPosPart(double x, double y, double w, double &xtr, double &ytr,
                double &wtr)
{
    if (abs(wtr + w) < 1e-10)
...
void AddNegPart(double x, ouble y, double w, double &xtl, double &ytl,
                double &wtl)
{
    if (abs(wtl + w) < 1e-10)
        return; // detect zero regions
...
void AddRegion(double x1, double y1, double x2, double y2, double &xtr,
               double &ytr, double &wtr, double &xtl, double &ytl,
               double &wtl)
{
    if (abs(x1 - x2) < 1e-10)
        return;
    ...
    if (x2 > x1) {
        AddPosPart((x2 + x1) / 2, y1 / 2, (x2 - x1) * y1, xtr, ytr,
                   wtr); /* rectangle 全局变量变化处 */
        AddPosPart((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3,
                   (x2 - x1) * (y2 - y1) / 2, xtr, ytr, wtr);
        // triangle 全局变量变化处
    } else {
        AddNegPart((x2 + x1) / 2, y1 / 2, (x2 - x1) * y1, xtl, ytl, wtl);
        // rectangle 全局变量变化处
        AddNegPart((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3,
                   (x2 - x1) * (y2 - y1) / 2, xtl, ytl, wtl);
        // triangle  全局变量变化处
    }
}
POINT cg_simple(int vcount, POINT polygon[])
{
    ...
    for (int i = 0; i < vcount; i++) {
        ...
        AddRegion(p1.x, p1.y, p2.x, p2.y, xtr, ytr, wtr, xtl, ytl,
                  wtl); //全局变量变化处
    }
...
POINT gravitycenter(int vcount, POINT polygon[])
{
    ...
    for (int i = 1; i < vcount - 1; i++) {
        ...
        y0 = (polygon[0].y + polygon[i].y + polygon[i + 1].y) /
             3; //求当前三角形的重心
...
/***********************************************************************************************/
void pointtangentpoly(int vcount, POINT polygon[], POINT p, POINT &rp,
                      POINT &lp)
{
    LINESEG ep, en;
    bool blp, bln;
...
// 调用ptoldist函数，在第4页
bool CircleRecRelation(POINT pc, double r, POINT pr1, POINT pr2, POINT pr3,
                       POINT pr4)
{
...
double P2planeDist(double x, double y, double z, double a, double b, double c,
                   double d)
{
...
void reflect(double a1, double b1, double c1, double a2, double b2, double c2,
...
double c2area(POINT p1, double r1, POINT p2, double r2)
{
    ...
    double s = 0;
    if (rr < r2) //相交弧为优弧
        s = r1 * r1 * (PI - sita1 / 2 + sin(sita1) / 2) +
            r2 * r2 * (sita2 - sin(sita2)) / 2;
    else //相交弧为劣弧
...
//圆和直线关系：
// 0----相离 1----相切 2----相交
int clpoint(POINT p, double r, double a, double b, double c, POINT &rp1,
            POINT &rp2)
{
    int res = 0;
